...
module Test.Dawn.Compiler.MidIR.Verify (spec) where

import Prelude

import Prim as P

import Dawn.Compiler.Bytecode (lower)
import Dawn.Compiler.Bytecode as B
import Dawn.Compiler.Abi (PrimOp(..))
import Dawn.Compiler.MidIR (Rep(..), VerifyError(..), emptyDebug, verify)
import Dawn.Compiler.MidIR as M
import Dawn.Compiler.TypedCore (Ident(..), Literal(..), ModuleName(..), OpName(..), Qualified(..), RowKey(..), Symbol(..), TyName(..))
...
spec :: Spec Unit
spec = describe "Dawn.Compiler.MidIR.Verify » modules a lowering must not accept" do
