...
module Test.Dawn.Compiler.TypedCore.Annotation (spec) where
...
import Dawn.Compiler.TypedCore (CheckedGroup, DecisionTree(..), Expr(..), Ident(..), Literal(..), ModuleName(..), Occurrence(..), Qualified(..), RowEntry(..), RowKey(..), Symbol(..), TyName(..), Type(..), Typed, check, declare, declareAnnotated, emptyContext, envOf, exprAnnotation, infer, intTy, primSignature, pureFn, stringTy, typeOf, withAnnotation)
import Dawn.Compiler.TypedCore.Context (bindVar)
...
spec :: Spec Unit
spec = describe "Dawn.Compiler.TypedCore.Check » what checking records" do
