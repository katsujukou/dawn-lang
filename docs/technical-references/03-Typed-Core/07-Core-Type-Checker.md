...
12. That a `perform k.op` finds `k ↦ E τ̄` in the ambient row, and that its argument and result match the signature `Σ(E)` gives `op`. **The key selects the element and the payload selects the protocol**: `k` is never looked up in `Σ`. That a `handle` removes the one element its key names, and that its clauses exhaust `dom(Σ(E))` for the `E` of that element's payload, each operation once. That each clause is checked according to its form: a `full` clause binds a continuation and its body has the answer type, while a `fast` clause binds none and its body has the type its operation resumes with (D28). That a handler's region stands in the row its clauses are typed at and not in the row of the computation it handles, that a `readCell` or `writeCell` names a key the region declares, and that the region variable occurs in neither the answer type nor the residual row, which is what keeps a cell from outliving its `handle` (D36)
